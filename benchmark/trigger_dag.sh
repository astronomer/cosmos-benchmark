...
RUN_ID=$(airflow dags trigger $DAG_NAME --output json | tail -1 | jq '.[0].dag_run_id')
...
while true; do
    ...
    STATUS=$(airflow dags list-runs -d $DAG_NAME --output json | jq ".[] | select(.run_id == $RUN_ID) | .state")
    echo $STATUS
