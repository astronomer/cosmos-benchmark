...
* Apache Airflow Helm chart `1.21.0` (Airflow `3.2.0`), `astronomer-cosmos`,
...
* **The tail grew significantly vs the 2026-05-08 baseline** (prior
  benchmark in commit history). At `threads=8` we measured `61 ± 6 s`
  vs `14 s` previously; at `threads=16`, `126 ± 14 s` vs `52 s`.
